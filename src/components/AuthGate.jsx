  ...
  return (
    <div style={{ minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'var(--bg)' }}>
      <div style={{ background: 'var(--bg2)', border: '0.5px solid var(--border2)', borderRadius: 14, padding: 36, width: 380 }}>
        ...
        <input value={name} onChange={e => setName(e.target.value)} style={inputStyle} placeholder="e.g. prayag"
          onKeyDown={e => e.key === 'Enter' && handleEnter()} />

        {error && <div style={{ fontSize: 12, color: 'var(--red)', marginTop: 10 }}>{error}</div>}

        <button onClick={handleEnter} style={{ marginTop: 24, width: '100%', background: 'var(--gold)', border: 'none', color: '#0a0a0f', padding: '11px', borderRadius: 8, cursor: 'pointer', fontSize: 13, fontWeight: 500, fontFamily: 'DM Sans,sans-serif' }}>
          Enter
        </button>
      </div>
    </div>
...
const labelStyle = { display: 'block', fontSize: 10, color: 'var(--text3)', fontFamily: 'DM Mono,monospace', letterSpacing: '0.06em', textTransform: 'uppercase', marginBottom: 6 }
